  ...
  instance.connection.on('connect', err => {
    if (err) {
      if (instance.options.printErrors) {
        console.error('archive-tds: Database connection failed with', err);
        console.info(
          'archive-tds: Check the database configuration values in the environment variables'
        );
      }
      ...
      process.on('exit', code => {
        ...
        console.log(
          'archive-tds: Shutting down server: no longer listening on port'
        );
        if (instance.options.printErrors == false) {
          console.log(
            'archive-tds: Run with `printErrors=true` for more information'
          );
        }
      });
